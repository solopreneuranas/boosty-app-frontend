...
export default function AdminTickets() {
    ...
    const fetchTickets = async () => {
        ...
        var response = await postData('tickets/display_all_tickets', body)
        if (response?.status) {
            setLoading(false)
                    ...
                    <Fade in={status}>
                        <Box sx={style}>
                            ...
                            <Grid container spacing={1} style={{ display: 'flex', justifyContent: 'left', alignItems: 'left', }}>
                                <h3 style={{ fontWeight: 500, fontSize: 18 }}>{title}</h3>
                            </Grid>
    ...
    const displayTicketsList = () => {
        return (
            ticketsList.map((item, i) => {
                ...
                return (
                    <div style={{ width: '100%' }} onClick={() => handleEditTicket(item)}>
                        <Grid container spacing={1} style={{ borderBottom: i === ticketsList?.length - 1 ? '' : '1px solid gainsboro', cursor: "pointer", width: '100%', margin: 0, padding: '1%', background: 'white', display: 'flex', alignItems: 'center' }}>
                            <Grid item xs={1}>
                                {i + 1}
                            </Grid>
                            <Grid item xs={8} style={{ display: 'flex', alignItems: 'center' }}>
                                <h3 style={{ margin: 0, fontWeight: 500, fontSize: 14 }}>{item.title.charAt(0).toUpperCase() + item.title.slice(1)}</h3>
                            </Grid>
        ...
        <div style={{ width: '95%', padding: '2%', background: 'white', height: '100vh' }}>
            {
                loading ?
                    ...
                    <>
                        {editTicketModal()}
                        <div>
                            <h3 style={{ margin: 0, fontWeight: 600, fontSize: 20 }}>Tickets List</h3><br />
                            {
